...
//double click listener
    canvas.addEventListener('dblclick', function (e) {
        ...
        dataPool.forEach(function (value) {
            let Area = findArea(value[0][0], value[0][1], value[1][0], value[1][1], value[2][0], value[2][1]);
            let Area1 = findArea(value[0][0], value[0][1], pos.x, pos.y, value[2][0], value[2][1]);
            let Area2 = findArea(value[0][0], value[0][1], value[1][0], value[1][1], pos.x, pos.y);
            let Area3 = findArea(pos.x, pos.y, value[1][0], value[1][1], value[2][0], value[2][1]);
            if (Math.round(Area) === Math.round(Area1 + Area2 + Area3)) {
                let newList = [];
                let item = dataPool[dataPool.indexOf(value)];
...
//movement listener
    canvas.addEventListener('mousemove', function (e) {
        ...
        if (isActive && flag) {
            ...
            let item = dataPool[draggedTriIndex];
            let difX = endCoordinates.x - item[0][0] + changeDistance.x;
            let difY = endCoordinates.y - item[0][1] + changeDistance.y;
            item[0][0] += difX;
            item[0][1] += difY;
            item[1][0] += difX;
            item[1][1] += difY;
...
// function used to do translation
    function doDragTranslation(newx, newy) {
        ...
        let item = dataPool[draggedTriIndex];
        let difX = newx - item[0][0] + changeDistance.x;
        let difY = newy - item[0][1] + changeDistance.y;
        item[0][0] += difX;
        item[0][1] += difY;
        item[1][0] += difX;
        item[1][1] += difY;
...
// function to get current mouse position
    ...
    function drawTriangle(mode, x1, y1, x2, y2) {
        ...
        let distance = calculateLineDistance(x1, y1, x2, y2);
        //height of the triangle
        let height = 1.414 * (distance) * mode;
        //making a path
        context.beginPath();
        context.moveTo(x1, y1);
        context.lineTo(x1 + distance / 2, y1 + height);
